...
fun main() {
	...
	aocRun(puzzleInput) { input ->
		return@aocRun process(input) { passport ->
			val size = passport.size
			return@process (size == 8 || (size == 7 && !passport.any { it.first == PassportField.COUNTRY_ID }))
				&& passport.all { it.first.validate(it.second) }
		}
	}
}

private fun process(input: String, validator: (List<Pair<PassportField, String>>) -> Boolean): Int = input.split("\n\n")
	.map { passport ->
		val fields = keyValueRegex.findAll(passport)
			.map { PassportField.getByCode(it.groupValues[1])!! to it.groupValues[2] }
			.toList()
		if (validator(fields)) 1 else 0
	}
	.sum()

private enum class PassportField(val code: String) {
	BIRTH_YEAR("byr") {
		override fun validate(value: String): Boolean = validateYear(value, 1920..2002)
	},
	ISSUE_YEAR("iyr") {
		override fun validate(value: String): Boolean = validateYear(value, 2010..2020)
	},
	EXPIRATION_YEAR("eyr") {
		override fun validate(value: String): Boolean = validateYear(value, 2020..2030)
	},
	HEIGHT("hgt") {
		override fun validate(value: String): Boolean = heightRegex.matchEntire(value)?.let {
	...
	HAIR_COLOUR("hcl") {
		override fun validate(value: String): Boolean = colourRegex.matches(value)
	},
	EYE_COLOUR("ecl") {
		override fun validate(value: String): Boolean = EyeColour.getByCode(value) != null
	},
	PASSPORT_ID("pid") {
		override fun validate(value: String): Boolean = value.length == 9 && value.toIntOrNull() != null
	},
	COUNTRY_ID("cid") {
	...
	companion object {
		...
		private val colourRegex = Regex("#[0-9a-f]{6}")

		fun getByCode(code: String): PassportField? = values().find { it.code == code }
	}

	abstract fun validate(value: String): Boolean

	protected fun validateYear(value: String, range: IntRange): Boolean =
		value.length == 4 && value.toIntOrNull()?.let { it in range } ?: false
}

private enum class EyeColour {
	AMB,
	BLU,
	BRN,
	GRY,
	...
	companion object {
		fun getByCode(code: String): EyeColour? = values().find { it.name.equals(code, true) }
	}
}
