...
fun main() {
	aocRun(puzzleInput) { input ->
		return@aocRun input.split("\n\n").sumOf { answers ->
			val charSet = mutableSetOf<Char>()
			answers.replace("\n", "").forEach { charSet += it }
			return@sumOf charSet.size
		}
	}
	aocRun(puzzleInput) { input ->
		return@aocRun input.split("\n\n").sumOf { answers ->
			val perPerson = answers.split("\n").map { it.toCharArray().toSet() }
			var set = perPerson[0]
			(1 until perPerson.size).forEach { set = set.intersect(perPerson[it]) }
			return@sumOf set.size
		}
	}
}
