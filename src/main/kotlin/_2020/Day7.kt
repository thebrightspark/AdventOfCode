...
fun main() {
	aocRun(puzzleInput) { input ->
		...
		input.split("\n").forEach { rule ->
			val matcher = REGEX_MAIN.matchEntire(rule)!!
			val bag = matcher.groupValues[1]
			val contained = matcher.groupValues[2]
			map[bag] = if (contained == NO_BAGS)
				emptySet()
			else
				REGEX_CONTAINED.findAll(contained).map { it.groupValues[2] }.toSet()
		}
		return@aocRun map.keys.count { hasShinyGoldBag(map, it) }
	}
	aocRun(puzzleInput) { input ->
		val map = mutableMapOf<String, Set<Pair<Int, String>>>()
		input.split("\n").forEach { rule ->
			val matcher = REGEX_MAIN.matchEntire(rule)!!
			val bag = matcher.groupValues[1]
			val contained = matcher.groupValues[2]
			map[bag] = if (contained == NO_BAGS)
				...
				REGEX_CONTAINED.findAll(contained).map { it.groupValues[1].toInt() to it.groupValues[2] }.toSet()
		}
		return@aocRun countContainedBags(map, SHINY_GOLD)
	}
}

private fun hasShinyGoldBag(map: Map<String, Set<String>>, key: String): Boolean {
	val contained = map.getValue(key)
	if (contained.isEmpty())
		return false
	if (contained.contains(SHINY_GOLD))
		return true
	return contained.any { hasShinyGoldBag(map, it) }
}

private fun countContainedBags(map: Map<String, Set<Pair<Int, String>>>, key: String): Int {
	val contained = map.getValue(key)
	if (contained.isEmpty())
		return 0
	val recursiveSum = contained.sumOf { countContainedBags(map, it.second) * it.first }
	val containedSum = contained.sumBy { it.first }
	return recursiveSum + containedSum
}
