...
private val pattern = Pattern.compile("(\\d+)-(\\d+) (\\w): (\\w+)")

fun main() {
	aocRun(puzzleInput) { input ->
		var valid = 0
		parseInput(pattern, input) { matcher ->
			val range = (matcher.group(1).toInt()..matcher.group(2).toInt())
			val char = matcher.group(3)[0]
			val password = matcher.group(4)
			if (range.contains(password.count { it == char }))
				valid++
		}
		return@aocRun valid
	}
	aocRun(puzzleInput) { input ->
		var valid = 0
		parseInput(pattern, input) { matcher ->
			val pos1 = matcher.group(1).toInt() - 1
			val pos2 = matcher.group(2).toInt() - 1
			val char = matcher.group(3)[0]
			val password = matcher.group(4)
			val occurence1 = password[pos1] == char
			val occurrence2 = password[pos2] == char
			if ((occurence1 && !occurrence2) || (!occurence1 && occurrence2))
				valid++
		}
		return@aocRun valid
	}
