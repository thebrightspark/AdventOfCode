...
private val dateFormat = DateTimeFormatter.ofPattern("uuuu-MM-dd HH:mm")

fun main() {
    aocRun(puzzleInput) { input ->
        val records = parseRecords(input)

        // Find guard with most minutes of sleep
        val totalSleep = mutableMapOf<Int, Long>()
        var lastSleepStart: LocalDateTime? = null
        records.forEach { record ->
            when (record.type) {
                RecordType.SLEEP -> lastSleepStart = record.timestamp
                RecordType.AWAKE -> {
                    val period = Duration.between(lastSleepStart!!, record.timestamp)
                    totalSleep.compute(record.guardId!!) { _, mins -> (mins ?: 0) + period.toMinutes() }
                    lastSleepStart = null
                }
                else -> Unit
            }
        ...
        // Find minute during which the guard sleeps the most
        val sleepFrequencyByMin = mutableMapOf<Int, Int>()
        var lastSleepStartMin = 0
        records.filter { it.guardId == guardId && it.type != RecordType.SHIFT }.forEach { record ->
            when (record.type) {
                RecordType.SLEEP -> lastSleepStartMin = record.timestamp.minute
                RecordType.AWAKE -> {
                    (lastSleepStartMin until record.timestamp.minute).forEach {
                        sleepFrequencyByMin.compute(it) { _, count -> (count ?: 0) + 1 }
                    }
                }
                else -> Unit
            }
    ...
    aocRun(puzzleInput) { input ->
        ...
        // Map of Minute to GuardId and Frequency
        val sleepFrequency = mutableMapOf<Int, MutableMap<Int, Int>>()
        var lastSleepStartMin = 0
        records.filter { it.type != RecordType.SHIFT }.forEach { record ->
            when (record.type) {
                RecordType.SLEEP -> lastSleepStartMin = record.timestamp.minute
                RecordType.AWAKE -> {
                    (lastSleepStartMin until record.timestamp.minute).forEach {
                        sleepFrequency.compute(it) { _, map ->
                            map?.compute(record.guardId!!) { _, count -> (count ?: 0) + 1 }
                            return@compute map ?: mutableMapOf(record.guardId!! to 1)
                        }
                    }
                }
                else -> Unit
        ...
        // Get guard which is most frequently asleep on same minute
        var minute = 0
        var guardId = 0
        var count = 0
        sleepFrequency.forEach { min, map ->
            ...
            if (maxEntry.value > count) {
                ...
                count = maxEntry.value
            }
        }
        println("Min: $minute, Guard: $guardId, Count: $count")

        return@aocRun guardId * minute
    }
}
...
private fun parseRecords(input: String): List<Record> {
    val records = input.split("\n")
        .map {
            ...
            return@map Record(
                matcher.group("guardId")?.toInt(),
                LocalDateTime.from(dateFormat.parse(matcher.group("timestamp"))),
                RecordType.fromString(matcher.group("message"))
            )
        }
        .sortedBy { it.timestamp }
    // Ensure all records have their guardId set
    var lastGuardId = -1
    records.forEach { record ->
        record.guardId?.let {
            lastGuardId = it
        ...
        record.guardId = lastGuardId
    }
    return records
}

private enum class RecordType(val key: String) {
    SHIFT("begins shift"),
    SLEEP("falls asleep"),
    AWAKE("wakes up");

    companion object {
        fun fromString(input: String): RecordType = values().find { input == it.key }!!
    }
}

private data class Record(var guardId: Int?, val timestamp: LocalDateTime, val type: RecordType)
